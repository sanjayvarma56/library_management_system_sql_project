...
CREATE TABLE branch (
  ...
  ALTER TABLE branch
  MODIFY COLUMN contact_no VARCHAR(40);
...
('978-1-60129-456-2','To Kill a Mockingbird','Classic',6.00,'yes','Harper Lee','.B. Lippincott & Co.');
SELECT * FROM books;
...
COUNT(issues_id) as total_books_issued
FROM issued_status
GROUP BY issued_emp_id
...
USE LIB;
SELECT * FROM members;
SELECT MIN(reg_date), MAX(reg_date) FROM members;
SELECT * FROM members 
